    ...
    def writer(self) -> str:
        ...
        for i in _sintax:
            if type(i) == str:
                ...
                _add_element = self.chooser(
                                self.shuffler(
                                 self.voc_domain[i]))[
                                  self.voc_domain[i]]

                self.elements[self.voc_domain[i]] = _add_element
                self.letter += _add_element
...
def writer(get_lex: callable,
    ...
    #	+ Preposicion {durante}
    #   + Fecha
    return redator(get_lex,memor_view,sesgos)